        ...
        <>
            ...
            <div className="container_list_produtos">
                <div className="card_produtos_carinho" id="produtos_carinho">
                    {produtos.map((produto) => (
                        <div className="card_produto" key={produto.id_produto}>
                            ...
                            <div className="remove" onClick={() => {ProductStore.removeProduto(produto.id_produto); setProdutos(ProductStore.getProdutos())}} >
                ...
                <div className="valor_total">
                    <p>valor total = R$<span id="valortotal">{(ProductStore.valorTotal().toFixed(2))}</span> </p>
                </div>
                <button id="entrega" onClick={() => setEntrega(true)} style={{background:  entrega === true ? "green" : "antiquewhite"}}>entrega</button>
                <button id="mesa" onClick={() => setEntrega(false)} style={{background:  entrega === true ? "antiquewhite" : "green"}}>mesa</button>

                <div className="config">
                    <button onClick={() => {onTroca("Cardapio")}}>add mais </button>
